...
"""
@File    : predict.py
@Time    : 2021/12/26 21:15
@Author  : 吴鸿萍
@Email   : 1359266354@XXX.com
@Software: PyCharm
"""
...
batch_size = 64
class_num=6
image_path = "6disease_new/"
saveModelName='InceptionV6-5-26.h5'
names=os.listdir('./'+image_path+'/test/')
...
covn_base = tf.keras.applications.InceptionV3(weights='imagenet', include_top=False, input_shape=(im_width, im_height, 3))
model = tf.keras.Sequential()
model.add(covn_base)
model.add(tf.keras.layers.GlobalAveragePooling2D())
model.add(tf.keras.layers.Dense(class_num, activation='softmax'))
model.summary()
model.load_weights('./save_weights/'+saveModelName)
...
def plotPictrue(fpr,tpr,roc_auc):
    ...
    save_name=['./InceptionV3-Algorithm-to-0-5-class-disease-sizeClass.png']
    lens=1
    for temp_save_name in save_name:
        for i in range(6*(lens-1),6*lens):
