...
"""
@File    : predict.py
@Time    : 2021/12/26 22:56
@Author  : 吴鸿萍
@Email   : 1359266354@qq.com
@Software: PyCharm
"""
import traceback
...
def plotPictrue(fpr,tpr,roc_auc):
    ...
    save_name=['./GoogLeNet-Algorithm-to-0-5-class-disease-sizeClass.png']
    lens=1
...
def saveParamIndex(trueLabel,predictLabel,class_num=class_num):
    ...
    for i in range(class_num):
        ...
        for j in range(len(predictLabel)):
            if predictLabel[j]==trueLabel[j] and trueLabel[j]==i+1:
                TP+=1
            elif predictLabel[j]==trueLabel[j] and trueLabel[j]!=i+1:
                TN+=1
            elif predictLabel[j]!=trueLabel[j] and trueLabel[j]==i+1:
                FN+=1
            elif predictLabel[j]!=trueLabel[j] and trueLabel[j]!=i+1:
...
if __name__ == '__main__':
    imagesModelRes,nonLabel,accuary,trueLabel,abiliable=imagesModel()
